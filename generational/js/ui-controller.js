...
/* =========================================================
	...
	// URL is the source of truth for timeline scale; base page (no variant) = generational
...
/* ============================================================
   ...
   ============================================================ */
		...
		// =========================================================
		// GENERATIONAL NAV BUTTONS
		// ---------------------------------------------------------
		// Restored after migration: Element, Crisis, selected-cycle Conj,
		// and dedicated Saturn/Jupiter conjunction navigation.
		// All use the existing smooth glide path via timeState.navTargetDateUTC.
		// =========================================================
		...
		function glideToDateUTC(dateUTC) {
			if (!(dateUTC instanceof Date) || !Number.isFinite(dateUTC.getTime())) return;
			freezeTime();
			isPaused = false;
		...
		function stepTimeListMs(times, direction) {
			...
			const nowMs = getNavNowMs();
			if (direction > 0) {
				for (const t of sorted) if (t > nowMs + 1000) return t;
				return sorted[0];
			}
			for (let i = sorted.length - 1; i >= 0; i--) if (sorted[i] < nowMs - 1000) return sorted[i];
			return sorted[sorted.length - 1];
		}

		function getDatasetConjunctionTimesMs(p1, p2) {
			const keyA = `${p1}|${p2}`;
			const keyB = `${p2}|${p1}`;
			const ds = window.CONJUNCTION_DATASETS && (window.CONJUNCTION_DATASETS[keyA] || window.CONJUNCTION_DATASETS[keyB]);
			if (!ds || !Array.isArray(ds.events)) return [];
			return ds.events.map(e => Date.parse(e && e.t)).filter(Number.isFinite);
		}

		function getSelectedCycleConjunctionTimesMs() {
			let events = [];
			if (typeof window.getActiveConjunctionEvents === "function") {
		...
		if (nextConjBtn) nextConjBtn.addEventListener("click", () => {
			const target = stepTimeListMs(getSelectedCycleConjunctionTimesMs(), +1);
			if (target !== null) glideToDateUTC(new Date(target));
		});
		if (prevConjBtn) prevConjBtn.addEventListener("click", () => {
			const target = stepTimeListMs(getSelectedCycleConjunctionTimesMs(), -1);
			if (target !== null) glideToDateUTC(new Date(target));
		});
		if (nextSatJupConjBtn) nextSatJupConjBtn.addEventListener("click", () => {
			const target = stepTimeListMs(getDatasetConjunctionTimesMs("Saturn", "Jupiter"), +1);
			if (target !== null) glideToDateUTC(new Date(target));
		});
		if (prevSatJupConjBtn) prevSatJupConjBtn.addEventListener("click", () => {
			const target = stepTimeListMs(getDatasetConjunctionTimesMs("Saturn", "Jupiter"), -1);
			if (target !== null) glideToDateUTC(new Date(target));
		});
		...
		if (nextElementBtn) nextElementBtn.addEventListener("click", () => {
			...
			const target = getNextElementSwitch(new Date(getNavNowMs()), +1);
			if (target && Number.isFinite(target.tMs)) glideToDateUTC(new Date(target.tMs));
		});
		if (prevElementBtn) prevElementBtn.addEventListener("click", () => {
			if (typeof getNextElementSwitch !== "function") return;
			const target = getNextElementSwitch(new Date(getNavNowMs()), -1);
			if (target && Number.isFinite(target.tMs)) glideToDateUTC(new Date(target.tMs));
		});
...
/* =========================================================
  ...
  try {
    ...
    // Birth data (personal-only — generational just passes timestamp)
