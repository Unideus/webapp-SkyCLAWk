...
const EventsRenderer = {
  ...
  init() {
    ...
    window.addEventListener("resize", () => this.positionCategoryBarOnLifeline());
  ...
  buildCategoryBar() {
    ...
    const actionButtons = [
      ...
      { id: "lifeEventsBtn", label: "Life Events", title: "Life Events", glyph: "✦", color: "#FFD700", cls: "life-events-btn", onClick: () => {
        if (typeof PersonalLife !== "undefined" && PersonalLife.openEventModal) PersonalLife.openEventModal();
      }},
      { id: "localHistoryBtn", label: "Local History", title: "Local History", glyph: "✦", color: "#FFD700", cls: "local-history-btn", onClick: () => {
    ...
    actionButtons.forEach(action => {
      ...
      b.id = action.id;
      b.className = `cat-btn ${action.cls}`;
      b.dataset.action = action.id;
      b.title = action.title;
      b.innerHTML = `<span class="cat-btn-dot" style="background:${action.color};font-size:9px;">${action.glyph}</span><span class="cat-btn-label">${action.label}</span>`;
